...
function App() {
  let initialState = Number(window.localStorage.getItem('lightSet')) || 0;
  let { value: light, setFalse, setTrue } = useBooleanState(initialState === 1);

  const setOff = () => setFalse();
  const setOn = () => setTrue();

  let fillColor = light === true ? '#ffbb73' : '#000000';

  useEffect(() => {
    window.localStorage.setItem('lightSet', light ? '1' : '0');
  }, [light]);

  return (
    <div className="app">
      Light Bulb
      <p data-testid="lightState">{light ? 1 : 0}</p>
      <div>
...
function LightbulbSvg(props: { fillColor: string }) {
  return (
    /*
